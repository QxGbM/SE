...
public class MainWindow {
	...
	public String[][] findFriends(String id) {
		String friends = new DatabaseAccessor().findFriends(id);
	...
	private static DeckCard[] dc = new DeckCard[13];
	...
	final static class DeckCard {
		...
		public boolean specialPracticed = false;///new field!
		...
		public DeckCard(Card in, int k) {
			...
			if(id == 1010) {
				...
				String c_id = new Float(id).toString();


				specialPracticed = x.checkSpecialPractice(new Float(MainWindow.id).toString(), c_id.substring(0, c_id.length()-3) );
				System.out.println(specialPracticed);
			}
			else if(id == 2040) {
				x = new DatabaseAccessor();
				String c_id = new Float(id).toString();
	...
	public void loadCardPanel() {
		...
		addSelected.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//new content!
				else if((dc[selectedCard].id == 1010 ||dc[selectedCard].id ==2040)  && !dc[selectedCard].specialPracticed) {
